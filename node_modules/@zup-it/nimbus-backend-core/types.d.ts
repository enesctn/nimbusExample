 ...
 */
export declare type DynamicExpression<T> = State<T> | Operation<T>;
/**
 ...
 */
export declare type DeepExpression<T> = T extends (number | boolean | string | any[]) ? Expression<T> : (Expression<T> | {
    [K in keyof T]: DeepExpression<T[K]>;
});
export declare type HttpMethod = 'Get' | 'Patch' | 'Put' | 'Post' | 'Delete';
declare type InterpolatedTextAcceptableTypes = string | number | boolean | State<any> | Operation<any> | {
    [K: string]: InterpolatedTextAcceptableTypes;
};
export declare type InterpolatedText = InterpolatedTextAcceptableTypes | InterpolatedTextAcceptableTypes[];
/**
 * If T is an expression, returns the result of the expression, otherwise returns T
 */
export declare type ValueOrResultOfExpression<T> = T extends State<infer R> ? R : (T extends Operation<infer R> ? R : T);
/**
 * A number or a string that represents a number
 */
export declare type AnyNumber = number | `${number}`;
export {};
